...
~/.local/bin/jarvis_say "Evening, sir. Initiating end of day cognitive rollup. Compiling telemetry from today's operations."

# 1. Extract clipboard data
CLIPBOARD_DATA=$(copyq eval 'for(i=0; i<20; ++i) print(read(i) + "\n---\n")' 2>/dev/null)

if [ -z "$CLIPBOARD_DATA" ]; then
    ~/.local/bin/jarvis_say "No clipboard telemetry found for today. Disengaging."
    exit 0
fi
...
if [ -n "$SUMMARY" ]; then
    ~/.local/bin/jarvis_say "$SUMMARY"
    ~/.local/bin/jarvis_say "Daily architecture logged. Disengage and rest."
else
    ~/.local/bin/jarvis_say "Neural synthesis failed. The daily archive could not be compiled."
fi